...
const Notifications = () => {
  ...
  const fetchNotifications = async () => {
    setLoading(true);
    try {
      const [notifRes, interviewRes] = await Promise.all([
        api.get('/notifications'),
        api.get('/interview/history')
      ]);
      
      if (interviewRes.data.success) {
        setCompletedJobIds(new Set(interviewRes.data.data.filter(i => i.status === 'completed' && i.type === 'interview').map(i => i.job?._id)));
      }

      // Map backend data to frontend expected structure
      const mapped = notifRes.data.data.map(n => {
        // Extract jobId from actionUrl if possible (/student/interview/:jobId)
        let jobId = null;
        if (n.actionUrl?.includes('/student/interview/')) {
          jobId = n.actionUrl.split('/').pop().split('?')[0];
        }
      ...
      <div className="max-w-5xl mx-auto space-y-10 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-4 sm:px-0">
          <div>
            ...
            <p className="text-slate-500 text-sm">Stay updated on your application status and latest opportunities</p>
          </div>
            ...
            <AnimatePresence mode="popLayout">
              {filteredNotifications.map((notif) => (
                <NotificationCard 
                  key={notif.id}
                  notification={notif}
                  isCompleted={notif.jobId && completedJobIds.has(notif.jobId)}
