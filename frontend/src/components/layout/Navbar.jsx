...
const Navbar = () => {
  ...
  return (
    <motion.nav
      ...
      className={`fixed w-full z-50 top-0 left-0 transition-all duration-300 ease-in-out border-b ${scrolled
          ...
          <div className="hidden lg:flex absolute left-1/2 transform -translate-x-1/2 items-center space-x-8">
            {navLinks.map((link) => (
              <Link key={link.name} to={link.path} className="relative group text-sm font-medium text-slate-300 hover:text-white transition-colors">
                {link.name}
                <span className={`absolute -bottom-1 left-0 h-0.5 bg-brand-400 transition-all duration-300 ${location.pathname === link.path ? 'w-full' : 'w-0 group-hover:w-full'
                  }`}></span>
              </Link>
            ))}
          </div>
            ...
            <div className="px-4 py-6 flex flex-col space-y-4">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.path}
                  className={`block text-lg font-medium transition-colors ${location.pathname === link.path ? 'text-brand-400' : 'text-slate-300 hover:text-white'
                    }`}
                >
                  {link.name}
                </Link>
